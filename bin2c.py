...
def bin2c(filename, varname='data', linesize=80, indent=4):
    ...
    out = '// Generated by bin2c.py on %s from "%s"\n\n' % (datetime.date.today(), filename)
    out += '#define %s_LEN %d\n' % (varname.upper(), len(data))
    out += 'const char %s[%s_LEN] = {\n' % (varname, varname.upper())
    line = ''
    for byte in data:
    ...
    if len(line) + indent + byte_len < linesize:
        out += ' ' * indent + line + '\n'
    # strip the last comma, add closing bracket and newline
    out = out.rstrip(', \n') + '\n'
    out += '};\n'
    return out
...
def main():
    ...
    try:
        with open(args.filename.rsplit(".", 1)[0] + '.c', 'w') as out_file:
            out_file.write(bin2c(args.filename, args.varname, args.linesize,
                                 args.indent))
            print('File "%s.c" is created.' % args.filename.rsplit(".", 1)[0])
